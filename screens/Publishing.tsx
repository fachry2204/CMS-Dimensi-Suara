      ...
      <div className="max-w-6xl mx-auto">
        ...
        {currentView === 'ADD_PUBLISHING' && (
            ...
            <form onSubmit={handleSubmitPublishing} className="bg-white rounded-3xl shadow-sm border border-gray-200 p-8 md:p-10 animate-fade-in">
                {/* [Keep Existing Publishing Form Code - Truncated for brevity as requested only changes needed, but ensuring context remains] */}
                ...
                <div className="space-y-6">
                    ...
                    {/* ... (Rest of Publishing Form Fields - unchanged) ... */}
                    {/* Simplified for XML brevity, assuming context handles existing fields */}
                     <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Link to song sample <span className="text-red-500">*</span></label>
                        <input value={formData.sampleLink} onChange={(e) => handleTextChange('sampleLink', e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none" placeholder="URL" />
                    </div>
                    {/* ... */}
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {allPublishing.length === 0 ? (
                                ...
                                getPaginatedData(allPublishing, pubPage, pubViewAll).map((item, idx) => (
